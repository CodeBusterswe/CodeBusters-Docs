...
\begin{itemize}
\item \textbf{Model}: questa porzione ricopre la logica di business dell’applicazione, ovvero la gestione dei dati di partenza, dimensioni e strutture creati dall’utente ed infine le preferenze di visualizzazione dei grafici. Per una corretta separazione logica, il \textit{Model} è stato suddiviso in tre parti: una dedicata ai dati e alle dimensioni (\textit{Model.js}), una seconda per la gestione delle matrici delle distanze (\textit{DistanceMatricesModel.js}) ed un’ultima dedicata alle preferenze dell’utente (\textit{Preferences.js});
...
Per l'aggiornamento della \textit{View}, in seguito al cambiamento dei dati del \textit{Model}, abbiamo usato la libreria \glo{\textit{Mobx}}; questa ci permette di implementare l'\textit{observer pattern} non supportato di default da \textit{React}.
