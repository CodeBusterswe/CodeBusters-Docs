...
Questa applicazione deve essere costruita con tecnologia \glo{server-less} con una \glo{API} basata su \glo{HTTP}:
\begin{itemize}
\item	Si utilizzano \glo{AWS} \glo{Lambda} e altre componenti di supporto (\glo{AWS API Gateway}, \glo{AWS DynamoDB}, \glo{AWS S3});
\item	\glo{CloudFormation} per la gestione delle risorse sopraelencate;
\item	\glo{Serverless Framework} per l’implementazione di applicazioni \glo{serverless}, facilitandone alcune difficili strutture (come permessi, sottoscrizioni o accesso);
